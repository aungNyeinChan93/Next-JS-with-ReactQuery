...
export async function GET(request: NextRequest) {
    return NextResponse.json({ users: demo_users.reverse() }, { status: 200 })
};
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!name || !email) {
            throw new Error('Some fileds is required')
        };
        ...
        return NextResponse.json({ newUser, users: demo_users }, { status: 201 })
    } catch (error) {
        return NextResponse.json({ error: error instanceof Error ? error.message : 'create user fail' })
    }
}
